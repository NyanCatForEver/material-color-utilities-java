...
public final class Contrast {
   ...
   * color's lightness to in order to reach their desired contrast, instead of guessing {@literal &} checking
   * with hex codes.
   */
  ...
  /**
   * Returns T in HCT, L* in L*a*b* {@literal >=} tone parameter that ensures ratio with input T/L*. Returns -1
  ...
  /**
   * Tone {@literal >=} tone parameter that ensures ratio. 100 if ratio cannot be achieved.
   *
  ...
  /**
   * Tone {@literal <=} tone parameter that ensures ratio. 0 if ratio cannot be achieved.
   *
   * <p>This method is unsafe because the returned value is guaranteed to be in bounds, but, the in
   * bounds return value may not reach the desired ratio.
   *
